...
impl CFImageService {
    ...
    pub async fn upload_from_url(&self, url: &str, purpose: ImageType) -> anyhow::Result<Url> {
        let api_url = format!(
    ...
    fn gen_custom_id(&self) -> String {
        format!("{}/{}", self.game_id, Uuid::new_v4())
    }

    pub async fn request_upload_image(&self, purpose: ImageType) -> anyhow::Result<PendingImage> {
        let api_url = format!(
        ...
        Ok(PendingImage {
            ...
            final_url: self.get_final_url(response)?,
        })
    }
