...
export function GameLog(props: { log: T.GameLog; creatures: Map<T.CreatureID, T.Creature> }) {
  ...
  if ("ChatFromGM" in log)
    return <div>&lt;GM&gt;&nbsp;{log.ChatFromGM}</div>;
  if ("ChatFromPlayer" in log)
    return <div>&lt;{log.ChatFromPlayer[0]}&gt;&nbsp;{log.ChatFromPlayer[1]}</div>;
  ...
  if ("CreateFolder" in log)
    return <div><div>Created Folder</div><div>{T.folderPathToString(log.CreateFolder)}</div></div>;
  if ("RenameFolder" in log)
    return <div>Renamed Folder {T.folderPathToString(log.RenameFolder[0])} to {log.RenameFolder[1]}</div>;
  if ("DeleteFolderItem" in log)
    return <div>Deleted folder item in {T.folderPathToString(log.DeleteFolderItem[0])}</div>;
  if ("MoveFolderItem" in log)
    return <div>Moved folder item from {T.folderPathToString(log.MoveFolderItem[0])}
      to {T.folderPathToString(log.MoveFolderItem[2])}</div>;
  if ("CopyFolderItem" in log)
    return <div>Copied folder item from {T.folderPathToString(log.CopyFolderItem.source)}
      to {T.folderPathToString(log.CopyFolderItem.dest)}</div>;
  if ("CreateItem" in log)
    return <div>Created item {log.CreateItem[1].name} in {T.folderPathToString(log.CreateItem[0])}</div>;
  if ("EditItem" in log)
    return <div>Edited item {log.EditItem.name}</div>;
  if ("CreateNote" in log)
    return <div>Created note {log.CreateNote[1].name}</div>;
  if ("EditNote" in log)
    return <div>Edited note {log.EditNote[2].name}</div>;
  if ("TransferItem" in log)
  ...
  if ("EditSceneDetails" in log)
    return <div>Edited details of scene {log.EditSceneDetails.details.name}</div>;
  ...
  if ("AddSceneChallenge" in log)
    return <div>Added a challenge to a scene: {log.AddSceneChallenge.description}</div>;
  if ("RemoveSceneChallenge" in log)
    return <div>Removed challenge from a scene: {log.RemoveSceneChallenge.description}</div>;
  ...
  if ("SetCreaturePos" in log)
    return <div>Set a creature position to {log.SetCreaturePos[2].toString()}</div>;
