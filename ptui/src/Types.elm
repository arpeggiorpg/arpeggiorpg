...
creatureDecoder =
  P.decode Creature
    ...
    -- The following are optional for dumb reasons:
    -- They are not really intrinsic data members of Creature objects,
    -- but rather only generated by the server on the fly. Any other representation
    -- of Creature (such as the one we send in EditCreature commands) do not contain these fields.
    -- By making these optional, we can decode Creatures in other contexts, such as:
    -- * in snapshots
    -- * In GameLogs like CreatureCreature
    -- * when decoding GameCommands from the TypeScript code.
    |> P.optional "can_act" JD.bool True
    |> P.optional "can_move" JD.bool True
...
-- this decoder only exists to verify the encoders in TypeScript
gameCommandDecoder : JD.Decoder GameCommand
gameCommandDecoder = sumDecoder "GameCommand" []
  [("EditCreature", JD.map EditCreature creatureDecoder)]
