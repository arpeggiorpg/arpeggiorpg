...
impl<T> FolderTree<T> {
    pub fn new(root: T) -> FolderTree<T> {
        ...
        FolderTree {
            nodes: HashMap::from_iter(vec![(path, (root, HashSet::new()))]),
        }
    }
    ...
    pub fn move_folder(
        ...
        if new_parent.is_child_of(path) {
            return Err(FolderTreeError::ImpossibleMove(
                path.clone(),
                new_parent.clone(),
            ));
        }
    ...
    fn get_data(&self, path: &FolderPath) -> Result<&(T, HashSet<String>), FolderTreeError> {
        self.nodes
            .get(path)
            .ok_or_else(|| FolderTreeError::FolderNotFound(path.clone()))
    }
...
impl FolderPath {
    ...
    pub fn root() -> FolderPath {
        FolderPath::from_vec(vec![])
    }
...
impl<'a, T: Serialize> Serialize for ChildrenSerializer<'a, T> {
        ...
        S: Serializer,
    {
        ...
        for child in children {
            ...
            let data = self.tree.get(&full_path).expect(
                "Child node should definitely exist here, since get_children() returned it",
            );
...
impl<T> DeserializeHelper<T> {
    fn into_folder_tree(self) -> FolderTree<T> {
        ...
        let first = iter.next().expect(
            "There must always be at least one element if this data structure exists... right?",
        );
