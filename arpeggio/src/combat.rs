...
impl<'game> DynamicCombat<'game> {
    ...
    pub fn next_turn(&self) -> Result<ChangedCombat<'game>, GameError> {
        let change = self.change_with(CombatLog::EndTurn {
            creature_id: self.current_creature()?.id(),
        })?;
        Ok(change)
    }
...
impl<'game> CombatMove<'game> {
    pub fn movement_left(&self) -> u32units::Length {
        self.movement_left
    }
...
pub mod test {
    ...
    pub fn t_act(
        ...
        perf(game, GMCommand::CombatAct { ability_id, target })
    }
    ...
    fn multiple_effects_per_target() {
        ...
        game.classes
            .mutate(&classid_rogue(), |r| r.abilities.push(abid_multi));
    ...
    fn move_some_at_a_time() {
        ...
        let game = t_perform(
            ...
            GMCommand::PathCurrentCombatCreature {
                destination: Point3::new(500, 0, 0),
            },
        );
        assert_eq!(
            game.get_scene(t_scene_id())
                .unwrap()
                .get_pos(cid_rogue())
                .unwrap(),
            Point3::new(500, 0, 0)
        ...
        let game = t_perform(
            &game,
            GMCommand::PathCurrentCombatCreature {
                destination: Point3::new(1000, 0, 0),
            },
        );
        assert_eq!(
            game.get_scene(t_scene_id())
        ...
        match perf(
            &game,
            GMCommand::PathCurrentCombatCreature {
                destination: Point3::new(1100, 0, 0),
            },
        ) {
            Err(GameError::NoPathFound) => {}
            x => panic!("Unexpected result: {:?}", x),
    ...
    fn move_honors_path() {
        ...
        game.scenes
            .mutate(&t_scene_id(), move |s| s.terrain = terrain);
