...
// However, for 3d shapes this might get expensive... Probably more efficient to first, find
// potential targets within the radius, and then try to path to each target.
    ...
    fn items_within_volume<I: Clone + Eq + Hash>(
        ...
        match volume {
            ...
            Volume::Line { vector } => {
                ...
                let end = (
                    dest.x.get::<meter>() as isize,
                    dest.y.get::<meter>() as isize,
                );
    ...
    fn point3_neighbors(
        ...
        for &x in [-100, 0, 100].iter() {
            for &y in [-100, 0, 100].iter() {
                ...
                if is_open(terrain, neighbor)
                    && self.volume_fits_at_point(volume, terrain, neighbor)
                {
                    ...
                    if is_angle
                        && !is_open(terrain, Point3::from_quantities(neighbor.x, pt.y, pt.z))
    ...
    F: Fn(&CollisionData, &CollisionData) -> Option<R>,
{
    ...
    for (obj1, obj2, ..) in world.contact_pairs(true) {
        if let Some(r) = f(
            world.collision_object(obj1).unwrap().data(),
            world.collision_object(obj2).unwrap().data(),
        ) {
            results.push(r);
        }
        if let Some(r) = f(
...
fn volume_to_na_shape(volume: Volume) -> shape::ShapeHandle<f64> {
    match volume {
        Volume::Sphere(r) => {
            shape::ShapeHandle::new(shape::Ball::new(r.get::<centimeter>() as f64))
        }
        ...
        Volume::VerticalCylinder { .. } => {
            unimplemented!("volume_to_na_shape for VerticalCylinder")
        }
    }
}
