...
impl<'creature, 'game: 'creature> DynamicCreature<'creature, 'game> {
            ...
            .collect::<Vec<ConditionID>>()
        {
            match changes.creature.conditions[&condition_id].remaining {
                ...
                Duration::Rounds(remaining) => {
                    if remaining > 0 {
                        changes = changes.apply(&CreatureLog::DecrementConditionRemaining {
                            id: condition_id,
                        })?;
                    } else {
                        changes =
                            changes.apply(&CreatureLog::RemoveCondition { id: condition_id })?;
                    }
                }
            }
        }
    ...
    fn generate_energy(&self, nrg: Energy) -> Vec<CreatureLog> {
        let delta = self
            .creature
            .max_energy
            .saturating_sub(self.creature.cur_energy);
        if delta > Energy(0) {
            vec![CreatureLog::GenerateEnergy {
                energy: cmp::min(delta, nrg),
            }]
        } else {
            vec![]
        }
    ...
    fn damage(&self, expr: &Dice) -> Vec<CreatureLog> {
        ...
        if amt >= self.creature.cur_health {
            let mut logs = vec![];
            logs.push(CreatureLog::Damage {
                hp: self.creature.cur_health,
                rolls,
            });
            if !self
                .creature
                .conditions
                .values()
                .any(|ac| matches!(ac.condition, Condition::Dead))
            {
                logs.push(Self::apply_condition_log(
                    Duration::Interminate,
                    Condition::Dead,
                ));
            }
    ...
    fn heal(&self, expr: &Dice) -> Vec<CreatureLog> {
        ...
        vec![CreatureLog::Heal {
            hp: cmp::min(missing, amt),
            rolls: dice,
        }]
    }
...
impl CreatureExt for Creature {
    ...
    fn apply_log(&self, item: &CreatureLog) -> Result<Creature, GameError> {
        ...
        match *item {
            CreatureLog::Damage { ref hp, .. } => {
                new.cur_health = new.cur_health.saturating_sub(*hp)
            }
